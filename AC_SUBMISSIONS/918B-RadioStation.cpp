/*
	SubmissionId	:	34696044
	ContestId	:	918
	Index	:	B
	ProblemName	:	Radio Station
...
int main()
{
    ...
    cin>>n>>m;
    string s1,s2;
    vector<pair<string,string> > my_vec;
    for(i=0;i<n;i++)
    {
        cin>>s1;
        cin>>s2;
        s2=s2+";";
        my_vec.push_back(make_pair(s1,s2));
    }
    for(i=0;i<m;i++)
    {
        cin>>s1;
        ...
        for(j=0;j<n;j++)
        {
            if(my_vec[j].second==s2)
            {
                cout<<s1<<" "<<s2<<" #"<<my_vec[j].first<<"\n";
                break;
            }
        }
    }
